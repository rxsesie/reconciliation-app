...
statements_list = [
    "ENERO- 2025-UHC- DIANELKYS.csv",
    "FEBRERO- 2025- UHC- DIANELKYS.csv",
    "MARZO-2025-UHC-DIANELKYS.csv",
]
...
def read_client_list(client_list_path) -> List[ClientRecord]:
    """Read client list from Excel and create structured records."""

    if not os.path.exists(client_list_path):
        print(f"Error: File {client_list_path} not found.")
        return []

    wb = load_workbook(filename=client_list_path, read_only=True)
    sheet = wb.active
    records = []
...
def read_uhc(file_path) -> List[UHCRecord]:
    """Read UHC commission data from CSV file."""
    ...
    columns = [
        "Member Name",  # H
        "Original Effective Date",  # L
        "Member State",  # Q
        "Commission Action",  # T [new, pmpm = paid, new chargeback = chargeback]
        "Payment Period",  # U
        "Commission",  # X [amount]
    ]
    ...
    try:
        with open(file_path, mode="r", encoding="utf-8") as file:
            ...
            # Skip rows with missing data in required columns
            for row in reader:
                if all(
                    row[i] if i is not None and i < len(row) else None for i in indices
                ):
                    commission_str = row[indices[5]].replace("$", "").replace(",", "")
                    try:
                        commission = float(commission_str)
                    except ValueError:
                        print(f"Warning: Invalid commission value: {row[indices[4]]}")
                        commission = 0.0

                    record = UHCRecord(
                        member_name=row[indices[0]],
                        effective_date=row[indices[1]],
                        state=row[indices[2]],
                        action=row[indices[3]].lower(),
                        payment_period=row[indices[4]],
...
def compare_names(statements_paths, client_list_path, company):
    """Match UHC statement records with client list records using fuzzy matching."""
    # Read data
    client_records = read_client_list(client_list_path)
    uhc_records = []
    for statement in statements_paths:
        uhc_records.extend(read_uhc(statement))

    # Filter for UHC clients
    client_list_uhc = [c for c in client_records if c.company == company]

    matched_rows = []
    unmatched_clients = []
    not_in_client_list = uhc_records.copy()

    # Process each client and look for matches
    for client in client_list_uhc:
        ...
        for member_name in client.members_list:
            ...
            for uhc_record in uhc_records:
                # Use fuzzy matching to compare names
                ratio = fuzz.token_set_ratio(
                    member_name, uhc_record.member_name, processor=utils.default_process
                )

                # If good match found
                if ratio > 90:
                    matched_rows.append([uhc_record, client, member_name])
                    member_matched = True
                    client_matched = True
                    n_member_matched += 1
                    # Remove from not_in_client_list
                    if uhc_record in not_in_client_list:
                        not_in_client_list.remove(uhc_record)
...
def merge_records(matched_rows, unmatched_clients, not_in_client_list, company):
    ...
    for row in matched_rows:
        uhc_record = row[0]
        member_name = row[2]
        effective_date = uhc_record.effective_date
        state = uhc_record.state
        payment_period = uhc_record.payment_period
        commission = uhc_record.commission
        action = uhc_record.action
        in_client_list = True
    ...
    for uhc_record in not_in_client_list:
        member_name = uhc_record.member_name
        effective_date = uhc_record.effective_date
        state = uhc_record.state
        payment_period = uhc_record.payment_period
        commission = uhc_record.commission
